...
## Деплой на VPS / Облачный сервер (Beget)

Сейчас сайт крутится на Vercel. Если вместо этого решите разместить его на
своём сервере (например, Beget VPS/Облачный сервер) — учтите: обычный
**виртуальный хостинг** (тарифы «Блог», «Сайт» и т.п.) не подойдёт, там нет
возможности запустить постоянно работающий процесс Node.js. Нужен именно
VPS/Облачный сервер с root-доступом по SSH (у Beget при заказе такого
сервера есть готовый образ «Node.js» — с уже установленными Node.js, PM2 и
Nginx, это сильно упрощает первую настройку).

### Первая настройка (один раз)

1. Закажите VPS/Облачный сервер у Beget, при создании выберите образ
   «Node.js» (Ubuntu + Node.js + PM2 + Nginx уже установлены). Минимальной
   конфигурации (1 CPU / 1–2 ГБ RAM) для сайта такого масштаба достаточно.
2. Подключитесь по SSH и склонируйте репозиторий:
   ```bash
   git clone https://github.com/Remakerzzz/agalife.git
   cd agalife
   git checkout claude/agalife-scaffold-afisha-ia8fa3
   ```
3. Создайте на сервере файл `.env.local` с боевыми ключами Supabase (тот же
   формат, что в `.env.local.example`) — этот файл никогда не попадает в
   git, его нужно создать вручную командой `nano .env.local`.
4. Установите зависимости и соберите проект:
   ```bash
   npm install
   npm run build
   ```
5. Запустите через PM2 (конфиг уже есть в `ecosystem.config.js`):
   ```bash
   pm2 start ecosystem.config.js
   pm2 save
   pm2 startup   # чтобы сайт поднимался сам после перезагрузки сервера
   ```
   Сайт теперь слушает `127.0.0.1:3000`.
6. Настройте Nginx как обратный прокси — за основу возьмите
   `deploy/nginx.conf.example` (подставьте свой домен), включите конфиг и
   подключите бесплатный SSL через Let's Encrypt/certbot (Beget обычно
   помогает настроить это в панели при привязке домена).
7. Пропишите у домена A-запись на IP-адрес сервера.

### Обновление сайта после каждой новой порции правок

В отличие от Vercel, здесь автодеплоя по `git push` нет — после того как
смержите очередной bundle с изменениями и запушите в GitHub, на сервере
нужно вручную подтянуть и пересобрать:
```bash
cd agalife
bash deploy/redeploy.sh
```
Скрипт сам подтянет код, поставит зависимости, соберёт проект и перезапустит
процесс в PM2.

## Индексация в Яндексе

1. Убедитесь, что в Vercel (Project Settings → Environment Variables) задана переменная `NEXT_PUBLIC_SITE_URL` — ваш настоящий домен (например, `https://agalife.ru` или `https://agalife.vercel.app`).
2. Зайдите на [webmaster.yandex.ru](https://webmaster.yandex.ru), добавьте сайт.
3. Выберите способ подтверждения прав «Мета-тег» — Яндекс покажет код вида `1234567890abcdef`.
4. Добавьте его в Vercel как `NEXT_PUBLIC_YANDEX_VERIFICATION` (и в `.env.local` для локальной разработки), передеплойте сайт.
5. В Яндекс.Вебмастере нажмите «Проверить» — мета-тег на странице должен подтвердить права автоматически.
6. Там же, в разделе «Индексирование» → «Переобход страниц», добавьте главную страницу и несколько ссылок на события, чтобы ускорить первое сканирование.
7. Сайт уже отдаёт `robots.txt` и `sitemap.xml` автоматически (`/robots.txt`, `/sitemap.xml`) — Яндекс подхватит их сам через день-два, ручного шага для этого не требуется.
